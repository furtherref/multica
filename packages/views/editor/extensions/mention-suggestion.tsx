...
function MentionRow({
  ...
  if (item.type === "issue") {
    ...
    // in the suggestion list — they're still selectable. Closed by CATEGORY, so a custom status that behaves as done reads as
    // closed too (MUL-6243). `archive` (fork status #39) is its own category
    // and is closed as well.
    const isClosed =
      item.statusCategory === "done" ||
      item.statusCategory === "cancelled" ||
      item.statusCategory === "archive";
    return (
      <button
        type="button"
        ref={buttonRef}

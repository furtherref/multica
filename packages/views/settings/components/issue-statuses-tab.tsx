      ...
      <div className="divide-y divide-surface-border">
        {builtIn && (
          <BuiltInRow
            ...
            behavior={categoryBlurb(t, category)}
          />
        )}
...
/**
 * The behavior blurb for a category, or nothing for `archive`.
 *
 * `archive` (fork status #39) resolves as a category wherever a status key is
 * rendered, but it is not a CATALOG category — the server refuses it as one and
 * ALL_STATUSES omits it, so this tab never offers it and there is no blurb.
 * Taking the category as a plain argument is what makes the narrowing survive
 * into the selector callback below; a narrowed `draft.category` would not.
 */
function categoryBlurb(
  ...
  return t(($) => $.issue_statuses.categories[category]);
}
          ...
          <div className="space-y-2">
            ...
            <p className="text-caption text-muted-foreground">
              {status
                ...
                : categoryBlurb(t, draft.category)}
            </p>
          </div>
          <div className="space-y-2">

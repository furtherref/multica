...
import { ALL_STATUSES, STATUS_ORDER } from "@multica/core/issues/config";
...
export function useIssueSurfaceData({
  ...
  const visibleStatuses = useMemo<IssueStatusCategory[]>(() => {
    ...
    // STATUS_ORDER rather than ALL_STATUSES: `archive` (fork status #39) is a
    // column but not a catalog category, and it is opt-in — it appears only
    // while a status filter explicitly selects it, never in the default view.
    return STATUS_ORDER.filter(
      (s) =>
        !hiddenStatusCategories.includes(s) &&
        (selected === null ? s !== "archive" : selected.has(s)),
    );
  ...
  // rest of the statuses. `archive` is intentionally excluded from this toggle
  // set — it is opt-in only, via an explicit status filter, so offering it as
  // a "hidden column" to restore would be a second, contradictory way in.

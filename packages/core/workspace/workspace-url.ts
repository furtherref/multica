// Brand host shown as the workspace URL prefix on the managed FurtherRef
// deployment, and the fallback whenever the deployment exposes no app URL.
// `/api/config` deliberately omits `daemon_app_url` for the managed cloud (and
// for any self-hosted server that has not set MULTICA_APP_URL / FRONTEND_ORIGIN),
// so this literal must remain the ultimate fallback.
const BRAND_WORKSPACE_HOST = "multica.furtherref.com";
 ...
 * self-hosted instances show their own domain instead of `multica.furtherref.com`.
 * Falls back to the brand host when no app URL is configured.
 */

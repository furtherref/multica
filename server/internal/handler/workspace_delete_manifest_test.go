...
var workspaceDeletionManifest = map[string]workspaceDeleteAction{
	...
	// Fork-only, workspace-scoped (migration 108); the row goes with the
	// workspace via its ON DELETE CASCADE.
	"issue_template":             workspaceDelete,
	...
	// Fork-only skill translations; both cascade from `skill`, which this
	// manifest already deletes.
	"skill_display_translation":      workspaceDelete,
	"skill_display_translation_file": workspaceDelete,

...
4. Has the runtime's cost budget been reached — either the runtime total or the budget for the agent's owner? The run is refused with `budget_exceeded` and no task is created; the runtime detail page's Cost budget card shows which limit is spent. Only the runtime's owner can set or change that budget — workspace owners and admins cannot, and a runtime with no owner has nobody who can, so its card is read-only. Budgets reset on the UTC period boundary (the day, Monday, or the first of the month), and the person whose run was refused plus the runtime owner get a `runtime_budget_exceeded` inbox notice once per period. Every trigger is gated, not just the ones a person clicks: issue assignment, @mention, chat, quick create, autopilot (a `run_only` autopilot records a `skipped` run carrying `budget_exceeded`), the manual source-context retry, and the coordinator wake-up after a delegated worker failure. A reached budget also **suppresses automatic retries** — the parent stays failed with its own reason and no retry child appears — and a **deferred task that comes due against a spent budget is failed with `failure_reason = budget_exceeded`** rather than promoted, so it shows as a terminal outcome instead of waiting forever.

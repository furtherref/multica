...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if len(modelVisibleSkills(ctx.AgentSkills)) > 0 {
		b.WriteString("4. **Before writing any code, complete the Skills protocol in the `## Skills` section below** — read the `SKILL.md` of every skill matching this task and comply with its required rules. Then complete the task within your Agent Identity boundaries. Do not investigate, implement, create issues, update issues, or delegate if your Agent Identity forbids that action; if your role is delegation-only, perform the allowed delegation work and stop once that outcome is delivered.\n")
	} else {
	...
	// Same model-visible gate as step 4 above (see writeSkills).
	if len(modelVisibleSkills(ctx.AgentSkills)) > 0 {
		b.WriteString("- If a reply IS warranted: do any requested work first — **if that work involves writing, modifying, or reviewing code, complete the Skills protocol in the `## Skills` section below before you start** (read the `SKILL.md` of every skill matching the task and comply with its required rules). Then **decide whether to include any `@mention` link.** The default is NO mention. Only mention when you are escalating to a human owner who is not yet involved, delegating a concrete new sub-task to another agent for the first time, or the user explicitly asked you to loop someone in. Never @mention the agent you are replying to as a thank-you or sign-off.\n")
	} else {

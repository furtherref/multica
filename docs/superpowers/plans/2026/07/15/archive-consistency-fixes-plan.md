...
**Goal:** Make the fork-only `archive` issue status consistently mean "retired work": archiving (or creating into archive) never starts, allows, executes, or re-triggers agent runs — including under concurrency and via external webhooks — never blocks new issues as a duplicate, and never holds child-stage barriers open, so no new model spend can appear on retired issues.

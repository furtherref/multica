...
**Goal:** Make the fork-only `archive` issue status consistently mean "retired work": archiving (or creating into archive) never starts, allows, executes, or re-triggers agent runs — including under concurrency and via external webhooks (statement-level best-effort at the extreme concurrency margin — see the design addendum for the precise bound) — never blocks new issues as a duplicate, and never holds child-stage barriers open, so no new model spend can appear on retired issues.

**Architecture:** Eight small, independent behavioral fixes in the Go backend (`server/`): five guard existing decision points with the `archive` status, one adds DB-level execution guards (claim + retry predicates) that make the invariant race-proof at the single queued→dispatched chokepoint, one fixes the batch child-done parent guard, and one stops the GitHub PR webhook from resurrecting archived issues. No schema changes, no migrations, no new endpoints. One new wire-visible dispatch reason code (`issue_archived`) is added for the rerun/mention guards, with matching frontend copy in `packages/views` and built-in skill doc updates. All fixes are covered by DB-backed handler tests following the existing `internal/handler` test conventions.

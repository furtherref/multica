...
- Stale `mdt_` cache entries (a re-population race can let one outlive the deleted token by up to AuthCacheTTL) are neutralized rather than chased: every consequential daemon path — heartbeat liveness, task claim (HTTP single/batch and WS RPC), registration — re-checks the runtime owner's account status per request or per write.
- Daemon revocation frames ride the ScopeDaemonRuntime relay in EVERY relay mode, including `legacy` (which previously had no daemon-scope consumer and therefore only severed sockets on the suspending node).

...
- Cache-revocation failures are surfaced, not swallowed: after the transaction commits, the handler invalidates the account-status cache and the deleted daemon tokens' cache entries with one retry each; a persistent failure returns 500 (`account status updated but revocation propagation failed`) — the PATCH is idempotent, so retrying re-runs the invalidation against the committed status.

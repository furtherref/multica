...
- Multi-node deployments: this node's hub can only evict its local sockets, so the suspend path also publishes the typed `auth_error` frame through the relay-backed broadcaster — cooperating clients on other nodes terminate their own sessions. Residual: a non-cooperating client on another node keeps receiving read-only realtime events until its next reconnect, which the WS auth path rejects.

...
interface TabStore {
   ...
   *     (activating the opened/existing tab).
   ...
   *   - If a tab for the same resource already exists in the target
   *     workspace, activate it; otherwise add a new tab and activate it.
   */
    ...
    (set, get) => ({
      ...
      openWorkspaceTab(path, title, icon) {
        ...
        if (targetSlug === activeWorkspaceSlug) {
          return get().openTab(clean, title, icon, { activate: true });
        }
        ...
        // Same resourceKey dedupe as openTab: the existing tab keeps its own
        // url (its filters/anchor are its view state); we only focus it.
        const key = resourceKeyForUrl(clean);
        const match = existing.tabs.find((t) => t.resourceKey === key);
        if (match) {
          set({

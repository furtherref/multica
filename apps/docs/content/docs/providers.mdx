...
**Of the 14 tools, twelve consume `mcp_config`: Claude Code, CodeBuddy, Codex, Copilot, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, and Trae**. The other two (Antigravity, Pi) accept the field but **ignore it** — no error, no warning, the config just has no effect.

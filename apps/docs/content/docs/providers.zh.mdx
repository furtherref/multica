...
GitHub 出品。模型路由走你的 GitHub 账号权益——工具自己不做模型选择，由 GitHub 决定给你用哪个模型。skill 放 `.github/skills/` 是 GitHub CLI 的原生发现机制。MCP 配置通过 `--additional-mcp-config` 传入——在会话内叠加到宿主机的 `~/.copilot/mcp-config.json` 之上，而不是替换它。
...
**16 款工具里有 13 款实际消费 `mcp_config`：Claude Code、CodeBuddy、Codex、Copilot、Cursor、Hermes、Kimi、Kiro CLI、OpenCode、OpenClaw、Qoder、Trae CLI、Grok**。其他 3 款（Antigravity、DevEco Code、Pi）会接收这个字段但**忽略**——不报错、不警告，只是配置不生效。

各工具的接入方式不同：Claude Code 和 CodeBuddy 通过 `--mcp-config` 加 `--strict-mcp-config` 接收；Codex 会把 daemon 管理的 `mcp_servers` block 写入单次任务的 `$CODEX_HOME/config.toml`；Copilot 通过 `--additional-mcp-config` 接收单次任务的临时文件，叠加在宿主机 `~/.copilot/mcp-config.json` 之上；Cursor 会写入 `.cursor/mcp.json`，并把项目 approval 写到单次任务的 `CURSOR_DATA_DIR`；Hermes、Kimi、Kiro CLI、Qoder、Trae CLI、Grok 通过 ACP `mcpServers` 接收；OpenCode 通过 `OPENCODE_CONFIG_CONTENT` 环境变量内联接收；OpenClaw 通过 Multica 的单次任务配置 wrapper 接收 `mcp.servers`。OpenCode 这条路径**不会**改写项目里的 `opencode.json`。

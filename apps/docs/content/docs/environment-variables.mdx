...
| Variable | Default | Description |
|---|---|---|
| `GITHUB_TOKEN` | empty | Optional token that authenticates Multica's own calls to the GitHub REST API, raising the rate limit from 60 to 5000 requests/hour |

`GITHUB_TOKEN` is **unrelated** to the GitHub App integration above. It authenticates two GitHub REST API callers: **CLI self-update** release lookups (read by the **daemon**) and **skill imports** from GitHub repositories (read by the **server**). Unauthenticated GitHub API requests are capped at **60 requests/hour per source IP**, which is trivially exhausted when many daemons or tools share one egress IP (NAT / forward proxy) and surfaces as `403` errors — for example `fetch release metadata: GitHub API returned 403` during CLI self-update. Setting `GITHUB_TOKEN` raises the limit to **5000/hour scoped to the token** rather than the IP. Any token that can read the public release repo works; no scopes are required for public repositories.

Set it in the environment of whichever process needs it — the **daemon** for CLI self-update, the **server** for skill imports — and **restart that process**, since the token is read at startup.

...
The two are complementary. In Multica today, MCP support is **provider-specific**: every tool except Antigravity and Pi consumes `mcp_config`; those two receive the field but don't actually use it. A dedicated MCP section will come in a later release.

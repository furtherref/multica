...
/**
 * Canonical column order — the 7 categories plus the fork's `archive` (status
 * #39). Archive is deliberately absent from STATUS_CATEGORIES: the server
 * refuses it as a catalog category, so no custom status can inherit it and the
 * picker must not offer it as one. It still has to RESOLVE, or an archived
 * issue would fall through `statusCategoryOfKey` into Todo. Mirrors
 * `STATUS_ORDER` in packages/core/issues/config/status.ts.
 */
export const STATUS_ORDER: IssueStatusCategory[] = [...STATUS_CATEGORIES, "archive"];

const CATEGORY_SET = new Set<string>(STATUS_ORDER);
